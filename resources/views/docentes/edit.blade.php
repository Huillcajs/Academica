...
<h1 class="text-3xl sm:text-4xl font-display font-bold text-neon-purple mb-8 border-b-2 border-neon-purple/50 pb-2" 
    ...
    <i class="fas fa-user-edit mr-3"></i> Editar Docente: {{ $docente->user->name }}
</h1>

{{-- Manejo de Errores (Si estuvieran presentes) --}}
@if ($errors->any())
...
<form action="{{ route('docentes.update', $docente) }}" method="POST" class="space-y-6">
    ...
    <div class="space-y-2">
        ...
        <input type="text" id="name" name="name" value="{{ old('name', $docente->user->name) }}" required
    ...
    <div class="space-y-2">
        ...
        <input type="email" id="email" name="email" value="{{ old('email', $docente->user->email) }}" required
            class="w-full px-4 py-3 bg-dark-bg border border-gray-700/50 rounded-lg text-gray-200 placeholder-gray-500 focus:ring-neon-purple focus:border-neon-purple transition duration-300"
    ...
    {{-- Campo Código Docente (Deshabilitado para edición) --}}
    <div class="space-y-2">
        <label for="codigo" class="block text-sm font-medium text-gray-400">Código Docente</label>
        <input type="text" value="{{ $docente->codigo }}" disabled
            class="w-full px-4 py-3 bg-gray-800 border border-gray-700/50 rounded-lg text-gray-500 cursor-not-allowed"> 
    </div>

    {{-- Campo Especialidad/Área --}}
    <div class="space-y-2">
        <label for="especialidad" class="block text-sm font-medium text-gray-400">Especialidad/Área</label>
        <input type="text" id="especialidad" name="especialidad" value="{{ old('especialidad', $docente->especialidad) }}" required
            class="w-full px-4 py-3 bg-dark-bg border border-gray-700/50 rounded-lg text-gray-200 placeholder-gray-500 focus:ring-neon-purple focus:border-neon-purple transition duration-300"
    ...
    <div class="flex space-x-4 pt-4">
        <button type="submit" 
            ...
            <i class="fas fa-sync-alt mr-2"></i> Actualizar Docente
        </button>
